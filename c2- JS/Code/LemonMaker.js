...
// If the function is called with the argument true, it will output the names of the dishes 
// and calculate their final price (including 20% tax)

// If the function is called with the argument false, 
// it will output the names of the dishes and give their prices without the additional tax
...
function foo(withTax,...args){
   var firstLine= 'Prices ';

    if(withTax){
        firstLine=+ "without tax";
   }
   else  firstLine=+ "with 20% tax";
    for (i in args) {
        let end= '$ ';
        var price=args[i].price;
        if (withTax) price=price*args[i].tax;
        console.log('Dish: ',args[i].name,': ',end);
    }
